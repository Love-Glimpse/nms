...
/*    	 var chapter_num = $("#chapter_name").children('option:selected').val();
    	 var partner_id = $("#chapter_name").attr('data-part');
    	 var book_id = $("#book_name").children('option:selected').val();
    	 //var url = "http://c"+partner_id+".mzshu.com/kdread/chapter/"+chapter_id+"";
    	 var url = "http://c"+partner_id+".mzshu.com/readLog/"+book_id+"/"+chapter_num;
    	 var val = $("input[name='publish_type']:checked").val();
   ...
   $("#internalPush-modal").on('click', '#search_bookName', function(){
	   ...
	   $.ajax({
			...
			success : function(data) {
				...
				$("#chapter_name").html("<option value='no'>请先选择一章</option>");
...
*/        
   ...
   $("#dialog_keyword").on('click', '#btn-cs', function(){
	   ...
	   if(objExp.test(user_id)){
		   ...
		   if(subscribe_mode == 1){
			   var book_id = $("txt-url2").attr('data-bookId');
		   }else{
			   var book_id = $("txt-url").attr('data-bookId');
		   }
           if(nav == 1){
       	   ...
       	   var  msg_type = $("input[name='publish_type']:checked").val(),
			...
			},
			{
				...
				formatter : function(value, row, index) {
					if(row.send_status==0){
						return "<font clolr='yellow'>待发送</font>"
					}else if(row.send_status==1){
						return "<font clolr='green'>正在发送</font>"
					}else if(row.send_status==2){
						return "<font clolr='gray'>已发送</font>"
					}else if(row.send_status==-1){
						return "<font clolr='red'>发送失败</font>"
					}
        ...
        buttons: [{
            ...
            handler: function () {
	            	...
	            	var nav  =  $("input[name='publish_type']:checked").val();
	         	    ...
	         	    var  task_name = $("#add_task_name").val(),//任务名称
	         	   		 type = $("input[name='subscribe_mode']:checked").val(),//1 小说  2活动
	         	   	 	 vip_type = $("input[name='paid']:checked").val(), //2 不限  1已充值  0 未充值
	        	   		 description = $("#add_des").val(), //描述
	        	   		 send_time = $("#send_time").val(), //发送时间
	          	   	     sex = $("input[name='gender']:checked").val(); //3不限  1男 2女
	                ...
	                if(nav == 1){//图文
	            	   ...
	            	   var msg_type = "text";
	            	   $("#hide-text").trigger("click"); //调用  文本拼接方法
	            	   		var pic_url = "",
	            	   		    url = "",
	            	   		    title = des;
	               }
...
function del() {
	...
	$.messager.confirm({
		...
		height : 250,
		title : '操作提示',
		msg : '确定删除数据？</br></br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style="color:red">正在发送和已发送的不能删除！</span>',
		fn : function(r) {
			if (r) {
				$.ajax({
					async : false,
