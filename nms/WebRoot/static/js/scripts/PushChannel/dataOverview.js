...
function getUserStatsList(group_by_parent){
	...
	$.ajax({
		...
		data:{
			...
			group_by_parent:group_by_parent //是否按照渠道分组
		},
...
function stat_user_change(){
	var group_by_parent = 0;
	if($("#group_by_parent").prop('checked')){
		group_by_parent=0;
	}else{
		group_by_parent=1;
	}
	getUserStatsList(group_by_parent);
}
//查看子渠道数据
...
$("#stat-user-search").click(function(e) {
	stat_user_change();
}
);
