...
public class OpenUrlController {
	...
	public String sdfs(@PathVariable()String random,HttpServletRequest request) {
		...
		if (partnerPushUrl != null) {
			partnerPushUrlService.openUrl(partnerPushUrl,request);
	...
	@ResponseBody
	@RequestMapping(value = "temp/chapter/{chapterId}",produces=MediaType.IMAGE_PNG_VALUE  )
	public byte[] chapterPicture(@PathVariable()Integer chapterId,HttpServletRequest request,HttpSession session) throws Exception{
		return partnerPushUrlService.chapterPicture(chapterId);
