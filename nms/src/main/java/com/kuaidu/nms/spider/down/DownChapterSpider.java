...
public class DownChapterSpider implements PageProcessor {
		...
		try {
			...
			if (statusCode.startsWith("2") || statusCode.startsWith("3")) {
				...
				if (t.equalsIgnoreCase("1")) {// 书本连接
					...
					if (sConfig.getNext_page_reg()!=null) {//目录有下一页
						List<String> next_page_urls = page.getHtml().xpath(sConfig.getNext_page_reg()).links().all();
						for (String url : next_page_urls) {
							ChapterNamesAndUrls chapterNamesAndUrls = Utils.getChapterNamesAndUrlsFromUrl(url, sConfig, replaces);
							if (chapterNamesAndUrls!=null) {
								chapterNames.addAll(chapterNamesAndUrls.getNames());
								chapterUrls.addAll(chapterNamesAndUrls.getUrls());
							}
						}
					}
					...
					for (int i = sConfig.getBefore_count(); i < size - sConfig.getAfter_count(); i++) {
						if (set.add(chapterNames.get(i).replaceAll(Config.CHAPTER_FILTER_REGEX, "").trim())) {// 用set去重
							...
							if (chapterUrls.get(i).contains("rzlib.net")) {
								chapter_url = chapter_url.replaceAll("/b/\\d+", "/b/txt222");
							}
						...
						if (chapterUrlsRequests.size()>=4000 || (i == size - sConfig.getAfter_count() - 1)) {
							if (saveLists.size()>0) {
	...
	public static class ChapterNamesAndUrls {
		List<String> names;
		List<String> urls;
