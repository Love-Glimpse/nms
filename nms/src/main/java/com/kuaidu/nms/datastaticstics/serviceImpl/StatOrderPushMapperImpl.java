...
public class StatOrderPushMapperImpl {
	...
	public String getStatOrders(StatOrderPushByDay statOrderPushByDay) {
		...
		statOrderPush.setPartner_id(statOrderPushByDay.getPartner_id());
		...
		for (String date : dates) {
			...
			if (date.equalsIgnoreCase(sdf.format(Utils.getDateByDays(0)))) {//今日统计
				sMapper.statTodayOrders();
			}
			...
			for (int i = 0; i < sOrderPushs.size(); i++) {
				...
				sByDay.setPartner_id(sOrderPushs.get(i).getPartner_id());
...
/*				if (i == 6) {
	...
	public String getSubDateDetail(Integer parentId, Integer pushId,String sub_date) {
		// TODO Auto-generated method stub
		List<StatOrderPush> statOrderPushs = sMapper.getSubDateDetail(parentId,pushId,sub_date);
		return ResultData.toJsonString(statOrderPushs.size(), statOrderPushs);
	}
}
