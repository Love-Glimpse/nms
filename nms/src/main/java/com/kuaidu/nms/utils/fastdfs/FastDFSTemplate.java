package com.kuaidu.nms.utils.fastdfs;
...
public class FastDFSTemplate {
    ...
    */
	...
	public void deleteFile(String url) throws Exception {
		...
		// 上传完成后删除旧文件
		Map<String, String> map = Utils.getGroupIdAndPath(url);
		if (map == null)
		{
			Log4jUtil.getBusinessLogger().error("url have no params!");
		}
		String group = map.get("group");
		String path = map.get("path");
		Log4jUtil.getBusinessLogger().info("group="+group);
		Log4jUtil.getBusinessLogger().info("path="+path);
		if (group != null || path != null){
			deleteFile(group, path);
		}else {
			Log4jUtil.getBusinessLogger().error("url have no group or path!");
		}
	}
