		...
		.datagrid(
			{
				columns : [ [
...

					{
						...
						formatter : function(value, row, index) {
							...
							if (row.nick_name != null
									&& row.nick_name != '') {
								ret = "<span >"
										+ "<img style=\"width:18px;margin-right:5px;vertical-align: middle;\" src=\""
										+ row.picture
										+ "\">"
										+ "<a href=\"javascript:void(0)\"  onclick=\"self.parent.addTab(\'"
										+ row.nick_name
										+ "("
										+ row.user_id
										+ ")"
										+ "\',\'userManagement/userDetailInfo?user_id="
										+ row.user_id
										+ "\',\'icon-add\')\"\>"
										+ "<span>" + row.nick_name
										+ "</span>" + "<span>("
...

					{
						...
						formatter : function(value, row, index) {
							if (row.kl_flag == 0) {
								...
								return "<span style=\"color:red\">已扣</span>";
							}
					...
					},
					{
						...
						align : 'center',
						formatter : function(value, row, index) {
							return (row.product_price * row.quantity)
								.toFixed(2)
						}
					} ] ],
	...
	$.ajax({
		...
		success : function(data) {
			...
			for (var i = 0; i < ret.length; i++) {
				html = html + "<option value=" + ret[i].partner_id + ">"
					+ ret[i].partner_name + "</option>"; // 添加option
			}
			proxy.html(html);
		} // 请求成功后处理函数。
	});
