	...
	$("#updatePushUrl").click(function(){
		...
		$.ajax({
			type:"post",
  			...
  			dataType:'json',
  			data:{'name':name,_method:'put',cost:cost},
...
function pushUrlOrderDetail(push_id){
	self.parent.addTab(push_id+"--详情",'partnerUserOrder/partnerUserOrderDetailIndex?push_id='+push_id);
}
	   /**
     *  二维码模板
...
/*          foreground: "#C00",
    ...
    function getParamQrcode(name,push_id,chapter_id){
		$.ajax({
			type:"post",
