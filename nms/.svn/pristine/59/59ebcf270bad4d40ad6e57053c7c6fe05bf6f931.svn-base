...
public class GetBookUpdateJob {
	...
	public void INovelJob() {	
		if (ip.matches("192\\.168\\.1\\.111")) {
			...
			//从rzlib找书
			List<Book_list> blists =  wImpl.getNoSourceUrlBooks();
			Log4jUtil.getBusinessLogger().info(blists.size());
			for (Book_list book_list : blists) {
				Log4jUtil.getBusinessLogger().info(book_list.toString());
				String bookUrl = BooksQueryMapperImpl.getUrlFromRzlibStr(book_list.getBook_name());
				Log4jUtil.getBusinessLogger().info(bookUrl);
				if (bookUrl!=null&&!bookUrl.equalsIgnoreCase("")) {
					book_list.setBook_url(bookUrl);
					wImpl.updateBookUrl(book_list);
				}
